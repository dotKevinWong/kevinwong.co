...
export const Box2 = (props: { left: number, top: number, name: string }) => {
  const [diffX, setDiffX] = useState(0);
  const [diffY, setDiffY] = useState(0);
  const [isDragging, setIsDragging] = useState(false);
  const [box, setBox] = useState({ left: props.left + '%', top: props.top + '%', position: 'absolute', zIndex: 1, textAlign: 'center' });

  const mouseStart = (e: { screenX: number; currentTarget: { getBoundingClientRect: () => { left: number; top: number; }; }; screenY: number; }) => {
    setIsDragging(true);
    setDiffX(e.screenX - e.currentTarget.getBoundingClientRect().left);
    setDiffY(e.screenY - e.currentTarget.getBoundingClientRect().top);
  }

  const touchStart = (e: { touches: { 0: { screenX: number; screenY: number; }; }; currentTarget: { getBoundingClientRect: () => { left: number; top: number; }; }; }) => {
  ...
  const touchMove = (e: { touches: { 0: { screenX: number; screenY: number; }; }; }) => {
    if (isDragging) {
  ...
  const mouseMove = (e: { target: string; screenX: number; screenY: number; }) => {
    if (isDragging) {
      setBox({
  ...
  return (
    <Box as="section" flex="1" p="6" marginTop="6" overflow="auto" onMouseMove={mouseMove} onTouchMove={touchMove}>
        ...
        .yellow_box {
            ...
            width: 64px;
            height: 64px;
            background-image: url('/yellow_folder.png');
        ...
        `}</style>
        <Box style={box}>
        <Box className="yellow_box" onMouseDown={mouseStart} onMouseUp={dragEnd} onTouchStart={touchStart} onTouchEnd={dragEnd}></Box>
